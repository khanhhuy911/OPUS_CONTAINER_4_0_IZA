...
=========================================================*/
    ...
    msgs['COM12190'] = 'Role cannot assign to Menu Item.';
	...
	*/
	...
	msgs['COM130104'] = 'There is no contents to save.';
	// input message
	...
	msgs['COM130903'] = '{?msg1} was disapproved successfully.';
	// print message 
	...
	msgs['COM132501'] = 'As there is no result retrieved, it\'s impossible to  download in the [EXCEL] format.';
	msgs['COM132601'] = 'Data Saved Successfully!!';
	msgs['COM132701'] = 'There is no data to see detail.';
	msgs['COM132801'] = 'Please Install CX60_Plugin_u_setup.exe';
	msgs['COM132901'] = 'Excel data cannot exceed 10,000 rows';
	msgs['COM132902'] = 'Excel data cannot exceed {?msg1} rows';
	msgs['COM132903'] = 'The selected data is over {?msg1} rows.\nThis download may cause a critical system shutdown.\nDo you still want to continue?';
	...
	function ComShowMessage(sMsg,Detail){
        ...
        /*에러메시지 출력 시 확인|취소 버튼 중 취소버튼을 없애 달라는 요건을 위해 위 코드로 변경 혹시 모르는 경우를 위해 주석처리로 남겨둠*/
//		try {
//			if (sMsg.length < 1) return;
...
//         } catch(err) { ComFuncErrMsg(err.message); }
    ...
    function ComFuncErrMsg(err_msg) {
        ...
        if(err_msg.indexOf("500 error") >=0){
        	top.window.location = "/opuscntr/500error.html";
        	return false;
        }else if(err_msg.indexOf("Login : OPUS Container") >=0){
        	top.window.location = "/opuscntr/SignOn.do";
        	return false;
        }else if(err_msg.indexOf("Under Construction") >=0){
        	top.window.location = "/opuscntr/NoAuthority.htm";
        	return false;
        }else if(err_msg.indexOf("Not found page") >=0){
        	top.window.location = "/opuscntr/NotFound.html";
        	return false;
        }else if(err_msg.indexOf("501 Error") >=0){
   ...
   function showErrMessage(message)
   {
       ...
       if (iLen >= 1)
       {
           showMessage = ComReplaceStr(message,"<||>","\n");
           ComShowMessage(showMessage);
       } // end if
   }
